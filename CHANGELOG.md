...
- Both templates: the app's `inert` attribute was driven only by `mustUserConsent`, so the UI could lock itself inert waiting on consent even when the `USER_CONSENT` feature flag was disabled — it now also checks `this.features.USER_CONSENT.isActive()`
- Both templates: reworded the welcome section's English and French copy for clarity, replacing em dashes with periods or colons throughout the description, steps and title
